...
from pyrogram.errors import FloodWait, ChannelInvalid, ChatAdminRequired, PeerIdInvalid
...
async def load_vc_logger_status():
    ...
    try:
        ...
        for chat_id in enabled_chats:
            try:
                if vc_logging_status.get(chat_id) is True:

                    await asyncio.sleep(2)

                    if chat_id not in monitoring_tasks:
                        task = asyncio.create_task(check_and_monitor_vc(chat_id))
                        monitoring_tasks[chat_id] = task

            except Exception as e:
                LOGGER.error(f"Failed to restore VC logger for {chat_id}: {e}")
...
async def is_admin(chat_id: int, user_id: int) -> bool:
    try:
        ...
        return member.privileges is not None or member.status in ["creator", "administrator"]
    except Exception:
        return False
...
async def vclogger_command(_, message: Message):
    ...
    asyncio.create_task(auto_delete(message, 1))
    ...
    if not await is_admin(chat_id, user_id):
        ...
        msg = await message.reply(
            "❌ <b>ᴀᴅᴍɪɴ ʀᴇǫᴜɪʀᴇᴅ!</b>\n\n"
            "ᴏɴʟʏ ᴀᴅᴍɪɴs ᴄᴀɴ ᴄʜᴀɴɢᴇ "
            "ᴛʜᴇ ᴠᴄ ʟᴏɢɢᴇʀ sᴇᴛᴛɪɴɢs.",
            disable_web_page_preview=True
        )

        asyncio.create_task(auto_delete(msg, 7))
        return
    ...
    current_state_ui = to_small_caps(str(status if status is not None else "ɴᴏᴛ sᴇᴛ"))

    if len(args) == 1:

        text = (
            f"📌 <b>ᴄᴜʀʀᴇɴᴛ ᴠᴄ ʟᴏɢɢɪɴɢ sᴛᴀᴛᴇ:</b> <b>{current_state_ui}</b>\n\n"
            f"<b>ᴜsᴀɢᴇ:</b> {prefix_ui} <b>[ᴏɴ/ᴇɴᴀʙʟᴇ/ʏᴇs | ᴏғғ/ᴅɪsᴀʙʟᴇ/ɴᴏ]</b>\n\n"
            f"<b>sᴛᴀᴛᴜs:</b>\n"
            f"• <b>ᴏɴ</b> - ɢᴇᴛ ᴊᴏɪɴ/ʟᴇᴀᴠᴇ ɴᴏᴛɪғɪᴄᴀᴛɪᴏɴs ɪɴ ᴠᴄ\n"
            f"• <b>ᴏғғ</b> - ᴅɪsᴀʙʟᴇ ᴠᴄ ɴᴏᴛɪғɪᴄᴀᴛɪᴏɴs"
        )
        ...
        if arg in ["on", "enable", "yes"]:
            ...
            msg = await message.reply(
                f"✅ <b>ᴠᴄ ʟᴏɢɢɪɴɢ ᴇɴᴀʙʟᴇᴅ</b>\n\n"
                f"<b>ᴄᴜʀʀᴇɴᴛ sᴛᴀᴛᴇ:</b> <b>{to_small_caps(str(vc_logging_status[chat_id]))}</b>",
                disable_web_page_preview=True
            )
            ...
            msg = await message.reply(
                f"❌ <b>ɪɴᴠᴀʟɪᴅ ᴀʀɢᴜᴍᴇɴᴛ!</b>\n\n"
                f"ᴜsᴇ <b>[ᴏɴ/ᴇɴᴀʙʟᴇ/ʏᴇs | ᴏғғ/ᴅɪsᴀʙʟᴇ/ɴᴏ]</b>",
                disable_web_page_preview=True
            )
...
async def get_group_call_participants(userbot, peer):
    try:
        ...
        wait_time = e.value
        LOGGER.warning(f"Flood wait detected for {wait_time} seconds")
...
async def monitor_vc_chat(chat_id):
    ...
    try:
        ...
        if not userbot:
            LOGGER.warning(f"No assistant available for chat {chat_id}")
            return

    except Exception as e:
        LOGGER.error(f"Assistant fetch failed for {chat_id}: {e}")
        return
    ...
    while (
        ...
        try:
            ...
            participants_list = await get_group_call_participants(
                userbot,
                peer
            )
...
async def check_and_monitor_vc(chat_id):
    ...
    try:
        ...
        if not userbot:
            LOGGER.warning(
                f"No assistant found for VC logger in chat {chat_id}"
            )
            return

    except Exception as e:
        LOGGER.error(
            f"Assistant loading failed for chat {chat_id}: {e}"
        )
        return
    ...
    try:
        ...
        if chat_id in monitoring_tasks:
            ...
            old_task = monitoring_tasks.get(chat_id)
...
async def handle_user_join(chat_id, user_id, userbot):
    try:
        ...
        name = user.first_name or "sᴏᴍᴇᴏɴᴇ"
        ...
        asyncio.create_task(auto_delete(sent_msg, 7))
